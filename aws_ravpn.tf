...
resource "aws_security_group" "allow_internal_networks" {
  ...
  tags = {
    "Name" = "SG-Allow-Internal-Networks"
  }
}
...
resource "aws_route" "internal_routes" {
...
data "template_file" "asa_config" {
  ...
  vars = {
    ...
    default_gateway_management = cidrhost(aws_subnet.management_subnets[floor(count.index / var.instances_per_az)].cidr_block, 1)
  }
}
